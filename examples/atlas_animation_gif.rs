...
fn create_atlas(paks: Vec<String>, bspname: String) -> Result<bool, Box<dyn Error>> {
    ...
    let bspname = format!("maps/{}.bsp", bspname.clone());

    let bsp_data = vfs.read(bspname.clone(), None)?;
    ...
    let mut image = File::create(format!("{}_atlas.gif", map_name)).unwrap();
    let mut encoder = Encoder::new(&mut image, frame_width, frame_height, &palette_data).unwrap();
