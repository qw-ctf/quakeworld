...
mod environment;
pub use environment::{Environment, IntoEnvironment};
...
impl DataTypeReader<'_> {
    ...
    pub fn read_exact(&mut self, buf: &mut Vec<u8>) -> Result<()> {
        let n = buf.capacity();
    ...
    pub fn read_exact_generic_string(&mut self, buf: &mut Vec<u8>) -> Result<()> {
        let n = buf.capacity();
        trace_start!(self, format!("Vec<u8>[{}]", n));
    ...
    pub fn read_exact_string(&mut self, buf: &mut Vec<u8>) -> Result<()> {
        let n = buf.capacity();
        trace_start!(self, format!("Vec<u8>[{}]", n));
        for _ in 0..n {
            buf.push(<u8 as DataTypeRead>::read(self)?);
    ...
    pub fn set_env<T: IntoEnvironment>(&mut self, name: impl Into<String>, value: T) {
        self.env.insert(name.into(), value.into());
    }
    pub fn get_env(&mut self, name: impl Into<String>) -> Option<Environment> {
        self.env.get(&name.into()).cloned()
    }

    pub fn get_env_error(&mut self, name: impl Into<String>) -> Result<Environment> {
        let name = name.into();
        if let Some(v) = self.env.get(&name) {
            return Ok(v.clone());
        }
        Err(Error::EnvironmentVariableNotFound(name.clone()))
    }
    ...
    pub fn read_exact_generic_v2<U: DataTypeRead>(&mut self, buf: &mut Vec<U>) -> Result<()> {
        let n = buf.capacity();
...
pub trait DataTypeRead: Sized {
    fn read(_datatypereader: &mut DataTypeReader) -> Result<Self> {
...
//         return Err(Error::ReadSizeError(current_position, len, TYPE_SIZE as u64));
//         }
...
macro_rules! datatypereader_generate_base_type {
    ($($ty:ty), *) => {
        ...
        impl DataTypeRead for $ty {
            fn  [< read >] (datareader: &mut DataTypeReader,) ->  Result<$ty> {
                const TYPE_SIZE:usize = std::mem::size_of::<$ty>();
                let current_position: u64 = datareader.cursor.position();
                trace_start!(datareader, stringify!($ty));
                let len = datareader.cursor.get_ref().len() as u64;
